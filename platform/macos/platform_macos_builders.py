...
def generate_bundle(target, source, env):
    ...
    if env.editor_build:
        # Editor bundle.
        prefix = "oculus." + env["platform"] + "." + env["target"]
        if env.dev_build:
            prefix += ".dev"
        ...
        if target_bin != "":
            shutil.copy(target_bin, app_dir + "/Contents/MacOS/Oculus")
        ...
        # Template bundle.
        app_prefix = "oculus." + env["platform"]
        rel_prefix = "oculus." + env["platform"] + "." + "template_release"
        dbg_prefix = "oculus." + env["platform"] + "." + "template_debug"
        if env.dev_build:
            app_prefix += ".dev"
            rel_prefix += ".dev"
            dbg_prefix += ".dev"
        ...
        if not os.path.isdir(app_dir + "/Contents/MacOS"):
            os.mkdir(app_dir + "/Contents/MacOS")
        if rel_target_bin != "":
            shutil.copy(rel_target_bin, app_dir + "/Contents/MacOS/oculus_macos_release.universal")
        if dbg_target_bin != "":
            shutil.copy(dbg_target_bin, app_dir + "/Contents/MacOS/oculus_macos_debug.universal")
